...
ifeq (,$(strip $(simple@cache) $(sorted@cache) $(unique@cache)))
    $(warning Neither simple@cache nor sorted@cache nor unique@cache was assigned)
endif
ifneq (,$(strip $(foreach v,$(simple@cache) $(sorted@cache) $(unique@cache),\
...
-include $(file@cache)
$(foreach v,$(simple@cache),$(eval $v := $($v)))
...
$(file@cache) :
	...
	$(foreach v,$(simple@cache) $(sorted@cache) $(unique@cache),$(file >>$@,$v := $($v)))
	$(file >>$@,endif)
